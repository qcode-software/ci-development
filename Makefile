...
package: check-version
	...
	tar --strip-components=1 --exclude Makefile --exclude .github --exclude tcl \
	--exclude tests -xzvf v$(VERSION).tar.gz -C $(TEMP_PATH)
	# checkinstall
	...
	--requires "tcl,tcllib,html2text,curl,tclcurl" --replaces none --conflicts none \
	make install
install:
